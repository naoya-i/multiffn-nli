...
def print_insts(f, pairs, answers, logits, label_dict, genres = None):
    """
    Print the pairs for which the model gave a wrong answer,
    their gold label and the system one.
    """
    ...
    if genres == None:
        genres = ["N/A"] * len(answers)
        
    for pair, answer, logit, gn in izip(pairs, answers, logits, genres):
        label_str = pair[2]
        label_number = label_dict[label_str]
        sent1 = ' '.join(pair[0])
        sent2 = ' '.join(pair[1])
...
def print_acc_per_genre(pairs, answers, logits, label_dict, genres):
    """
    Print the pairs for which the model gave a wrong answer,
    their gold label and the system one.
    """
    ...
    for pair, answer, logit, gn in izip(pairs, answers, logits, genres):
        ...
        inst[gn] += [(answer == label_number, answer, label_number)]

    for gn in sorted(inst.keys()):
        print("{}|{:.3f}".format(gn, 1.0 * sum(map(lambda x: x[0], inst[gn])) / len(inst[gn])))
...
if __name__ == '__main__':
    ...
    if None != args.insts:
        with open(args.insts, "w") as f:
            print_insts(f, pairs, answers, logits, label_dict, genres)
